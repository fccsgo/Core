...
public void FC_OnServerLoaded()
{
    g_MySQL = FC_Core_GetMySQL();
...
public Action Timer_CheckCache(Handle timer)
{
    ...
    g_KVCache.ImportFromFile("addons/sourcemod/data/com.fccsgo.stats.playerdata.kv");
    ...
    g_KVCache = new KeyValues("PlayerData", "", "");
    g_KVCache.ExportToFile("addons/sourcemod/data/com.fccsgo.stats.playerdata.kv");
...
static void ImportCacheToDatabase(int uniqueid, int playtotal, int spectotal, int playonct, int playonte, int playoncta, int playontea, int jointime, int duration, const char[] ip, const char[] map)
{
    ...
    FormatEx(m_szQuery, 2048,  "INSERT INTO `k_analytics` VALUES (  \
                                DEFAULT,        \
                                %d,             \
                                %d,             \
                                %d,             \
