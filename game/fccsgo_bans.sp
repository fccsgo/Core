...
{
    RegPluginLibrary("FCCSGO-Bans");

    CreateNative("FC_Bans_BanClient",       Native_BanClient);
    CreateNative("FC_Bans_BanIdentity",     Native_BanIdentity);
...
{
    if(FC_Core_GetServerId() < 0)
        return false;
    
    Database db = FC_Core_GetMySQL();
    if(db == null)
    {
    ...
    if(admin > 0)
    {
        ...
        adminUserId = FC_Core_GetClientUId(admin);
    ...
    char m_szQuery[1024];
    FormatEx(m_szQuery, 1024, "INSERT INTO k_bans VALUES (DEFAULT, '%s', '%s', '%s', %d, %d, %d, %d, %d, %d, '%s', '%s', -1);", steamid, ip, nickname, GetTime(), length*60, btype, FC_Core_GetServerId(), FC_Core_GetSrvModId(), adminUserId, adminNick, bReason);
...
public void BanClientCallback(Database db, DBResultSet results, const char[] error, DataPack pack)
{
    ...
    char kickReason[256];
    FormatEx(kickReason, 256, "您已被服务器封锁,禁止进入游戏!\n类型: %s\n原因: %s\n到期: %s\n访问https://fccsgo.com/查看详细信息", g_banType[btype], reason, timeExpired);
...
bool Util_BanIdentity(int admin, const char[] steamIdentity, int btype, int length, const char[] reason)
{
    ...
    char m_szQuery[1024];
    FormatEx(m_szQuery, 1024, "INSERT INTO k_bans VALUES (DEFAULT, '%s', '%s', '%s', %d, %d, %d, %d, %d, %d, '%s', '%s', -1);", steamIdentity, ip, targetName, GetTime(), length*60, btype, FC_Core_GetServerId(), FC_Core_GetSrvModId(), adminUserId, adminNick, bReason);
...
{
    SMUtils_SetChatPrefix("[\x02BAN\x01]");
...
public void OnAllPluginsLoaded()
{
    ...
    PrintToServer("[FCCSGO-Bans]  Loading Admins list...");
}
...
public void OnRebuildAdminCache(AdminCachePart part)
{
    ...
    char m_szQuery[256];
    FormatEx(m_szQuery, 256, "SELECT a.* FROM k_admins a LEFT JOIN k_admsrv b ON a.aid = b.aid WHERE b.srv_id = '%d' OR b.mod_id = '%d'", FC_Core_GetServerId(), FC_Core_GetSrvModId());
...
public void OnClientDisconnect(int client)
{
    ...
    SQL_VoidQuery(db, "UPDATE k_admins SET lastseen = '%d' WHERE adminName = '%s'", GetTime(), nick);
}